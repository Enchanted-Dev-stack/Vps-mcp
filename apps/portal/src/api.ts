...
export const api={
  ...
  createFolder:(parentPath:string,name:string)=>request<{path:string;name:string;isGitRepository:boolean}>("/api/repositories/folders",{method:"POST",body:JSON.stringify({parentPath,name})}),
  ...
  interrupt:(chatId:string,reason?:string)=>request(`/api/chats/${chatId}/interrupt`,{method:"POST",body:JSON.stringify({reason})}),
