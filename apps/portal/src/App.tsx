...
import {
  Activity, ArrowLeft, Bot, Check, ChevronDown, CircleHelp, Code2, Copy, Eye, Folder, FolderGit2, FolderPlus, Hammer,
  Link2, ListTodo, Loader2, LogOut, MoreHorizontal, Paperclip, Plus, Send, Square, TerminalSquare, X
} from "lucide-react";
import { api, setCsrf } from "./api";
import type { Attachment, Chat, ChatDetail, Mode, Question, RepositoryBrowserResult, RunEvent, Workspace } from "./types";

const eventTypes=["agent.connected","agent.disconnected","activity","command.started","command.stdout","command.stderr","command.completed","files.changed","question.created","question.answered","run.completed","run.failed","run.interrupt.requested","run.cancelled"];
...
export function App(){
  ...
  const [composer,setComposer]=useState(""),[sending,setSending]=useState(false),[uploading,setUploading]=useState(false),[stopping,setStopping]=useState(false),[pendingAttachments,setPendingAttachments]=useState<Attachment[]>([]),[error,setError]=useState("");
  ...
  useEffect(()=>{if(!chatId)return;const after=events.at(-1)?.seq??0;const source=new EventSource(`/api/chats/${chatId}/events/stream?after=${after}`,{withCredentials:true});const receive=(raw:MessageEvent)=>{try{const e=JSON.parse(raw.data) as RunEvent;setEvents(prev=>prev.some(x=>x.seq===e.seq)?prev:[...prev,e]);if(["activity","command.started","command.completed","run.completed","run.failed","run.interrupt.requested","run.cancelled","question.created","question.answered"].includes(e.type))void refreshDetail(chatId)}catch{}};for(const t of eventTypes)source.addEventListener(t,receive as EventListener);return()=>source.close()},[chatId,events.at(-1)?.seq,refreshDetail]);
  ...
  async function stopRun(){if(!detail?.activeRun||stopping)return;setStopping(true);try{await api.interrupt(detail.id,"User pressed Stop in the portal");await refreshDetail()}catch(e){setError(e instanceof Error?e.message:String(e))}finally{setStopping(false)}}
...
function RepositoryPicker({onSelect,onClose}:{onSelect:(path:string)=>void;onClose:()=>void}){
  const [data,setData]=useState<RepositoryBrowserResult|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(""),[newFolder,setNewFolder]=useState("");
  ...
  return <Modal title="Select workspace folder" onClose={onClose}><div className="repo-picker">
    <div className="repo-picker-head">{data?.currentPath&&<button type="button" className="icon-btn repo-back" onClick={()=>void load(data.parentPath??undefined)} title="Back"><ArrowLeft size={15}/></button>}<div><b>{data?.currentPath??"VPS locations"}</b><small>Choose the folder this workspace should own. Git is optional.</small></div></div>
    ...
    {loading?<div className="modal-loading"><Loader2 className="spin" size={16}/>Loading folders…</div>:<>
      {!data?.currentPath?<div className="repo-list">{data?.roots.map(root=><button type="button" key={root.path} onClick={()=>void load(root.path)}><Folder size={16}/><span><b>{root.name}</b><small>{root.path}</small></span><ChevronDown className="repo-chevron" size={14}/></button>)}</div>:<>
        {<div className="repo-current">{data.isGitRepository?<FolderGit2 size={16}/>:<Folder size={16}/>}<span><b>{data.isGitRepository?"Git workspace folder":"Workspace folder"}</b><small>{data.currentPath}</small></span><button type="button" className="primary" onClick={()=>onSelect(data.currentPath!)}>Use this folder</button></div>}<div className="new-folder-row"><input aria-label="New folder name" value={newFolder} onChange={e=>setNewFolder(e.target.value)} placeholder="New folder name"/><button type="button" className="secondary" disabled={!newFolder.trim()} onClick={async()=>{try{const made=await api.createFolder(data.currentPath!,newFolder.trim());setNewFolder("");await load(made.path)}catch(err){setError(err instanceof Error?err.message:String(err))}}}><FolderPlus size={14}/>Create folder</button></div>
        <div className="repo-list">{data.entries.map(entry=><div className={`repo-row ${entry.isGitRepository?"is-repo":""}`} key={entry.path}><button type="button" className="repo-open" onClick={()=>void load(entry.path)}><span className="repo-icon">{entry.isGitRepository?<FolderGit2 size={16}/>:<Folder size={16}/>}</span><span><b>{entry.name}</b><small>{entry.isGitRepository?"Git repository":entry.path}</small></span></button><button type="button" className="repo-use" onClick={()=>onSelect(entry.path)}>Select</button></div>)}{!data.entries.length&&<div className="repo-empty">No subfolders here.</div>}</div>
      </>}
    </>}
    <div className="repo-picker-note">This is a VPS folder browser. The selected folder becomes the agent's default workspace root; Git can be initialized or cloned later.</div>
  </div></Modal>
}
    ...
    <Modal title="Workspaces" onClose={onClose}>
      {!creating ? <>
        ...
        <div className="field-group repository-field">
          ...
          <div className="repository-input"><input aria-label="Workspace folder" value={rootPath} onChange={e=>setRootPath(e.target.value)} placeholder="Select or create a folder on the VPS" required/><button type="button" className="secondary" onClick={()=>setBrowse(true)}><Folder size={14}/>Browse VPS</button></div>
